...
setup_shell() {
  ...
  elif [ "$CURRENT_SHELL" = "nu" ]; then
    CONF_FILE=$HOME/.config/nushell/env.nu
    ensure_containing_dir_exists = "$CONF_FILE"
    echo "Installing for Nushell. Appending the following to $CONF_FILE:"
    {
      echo ''
      echo '# fnm'
      echo 'load-env ("'"$INSTALL_DIR"'/fnm env" | bash'
      echo '| lines'
      echo '| str replace "export " ""'
      echo '| str replace -a '\''"'\'' ""'
      echo '| split column "="'
      echo '| rename name value'
      echo '| where name != "FNM_ARCH" and name != "PATH"'
      echo '| reduce -f {} {|it, acc| $acc | upsert $it.name $it.value }'
      echo ')'
      echo ''
      echo '$env.PATH = ($env.PATH'
      echo '| split row (char esep)'
      echo '| prepend $"($env.FNM_MULTISHELL_PATH)/bin"'
      echo '| prepend '"$INSTALL_DIR"
      echo ')'
    } | tee -a "$CONF_FILE"
  else
